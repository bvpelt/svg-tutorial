(function () {
  ...
  const projection$1 = geoMercator()
    .center([0, 54])
    .scale(3000)
    .rotate([0, 0]);
  ...
  const geoGenerator = geoPath().projection(projection$1);
  ...
  json("Gemeentegrenzen.json").then(function (geojson) {
    ...
    projection$1.fitSize([width, height], geojson); // adjust the projection to the features
